...
export default function DashboardPage() {
  ...
  // ── Lateral method distribution ──────────────────────────────────────────
  const methodDist = useMemo(() => {
    const counts: Record<string, number> = {};
    for (const c of myConfigs) {
      const m = (c.config as any)?.lateral?.method ?? "unknown";
    ...
    return Object.entries(counts)
      ...
      .map(([label, value]) => ({
        ...
        value,
        color: METHOD_COLORS[label] ?? "bg-zinc-500",
      }));
  }, [myConfigs]);
      ...
      {hasConfigs && (
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <ChartCard title="Lateral Method" icon={Wrench}>
              {methodDist.length > 0 ? (
                <HBar
                  data={methodDist}
                  colorFn={(l) =>
                    METHOD_COLORS[l.toLowerCase()] ?? "bg-zinc-500"
                  }
                />
              ) : (
