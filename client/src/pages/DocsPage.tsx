  ...
  "getting-started": [
    {
      ...
      content: (
        <>
          ...
          <UL
            items={[
              "Full 10-section config editor covering Vehicle, Lateral, Longitudinal, Speed Control, Lane Change, Navigation, Interface, Comma AI, and Advanced settings",
      ...
      content: (
        <>
          ...
          <UL
            items={[
              <>
                <strong className="text-zinc-200">Vehicle</strong> — Make,
                model, year, fingerprint source, cinematic intro
              </>,
              <>
                <strong className="text-zinc-200">Driving Personality</strong> —
                Profile (eco/normal/sport/traffic), longitudinal personality,
                per-profile accel/decel/gap
              </>,
              <>
                <strong className="text-zinc-200">Lateral Control</strong> —
                Method (Torque, PID, INDI, LQR) with full parameter sets for
                each
              </>,
              <>
                <strong className="text-zinc-200">Longitudinal Control</strong>{" "}
                — SP-Long vs stock, E2E toggle, smooth stop, jerk limits
              </>,
              <>
                <strong className="text-zinc-200">Speed Control</strong> — Speed
                Limit Control (SLC), Vision Turn Control (VTSC), Map Turn
                Control (MTSC)
              </>,
              <>
                <strong className="text-zinc-200">Lane Change</strong> —
                Auto-timer, nudgeless mode, BSM integration
              </>,
              <>
                <strong className="text-zinc-200">Navigation</strong> — NOO,
                OSM, Mapbox token, pre-load maps
              </>,
              <>
                <strong className="text-zinc-200">Interface</strong> — Dev
                overlays, screen brightness, sidebar, metric units
              </>,
              <>
                <strong className="text-zinc-200">Comma AI</strong> — Drive
                recording, upload settings, LDW, connect integration
              </>,
              <>
                <strong className="text-zinc-200">Advanced</strong> — Custom
                fingerprint, prebuilt binary, SSH key, developer mode
              </>,
            ]}
          />
          ...
          <P>
            ...
            current config as a <Code>.json</Code> file. The file format is
            versioned (<Code>exportVersion: 1</Code>) and validated on import.
          </P>
          <H3>Deep links</H3>
          <P>
        ...
        <>
          ...
          <UL
            items={[
              ...
              "Category (Daily Driver, Performance, Highway, etc.)",
              "Tags (multi-select)",
      ...
      content: (
        <>
          ...
          <UL
            items={[
              <>
                <Code>GET /api/configs?page=1&amp;limit=24</Code> — paginated
                list of your configs (returns{" "}
                <Code>{"{ configs, total, page, limit }"}</Code>)
              </>,
              <>
                <Code>POST /api/configs</Code> — create a new config
              </>,
              <>
                <Code>GET /api/configs/:id</Code> — get single config (owner
                only)
              </>,
              <>
                <Code>PUT /api/configs/:id</Code> — update config (increments
                version, saves snapshot)
              </>,
              <>
                <Code>DELETE /api/configs/:id</Code> — delete config
              </>,
              <>
                <Code>POST /api/configs/:id/share</Code> — publish config,
                returns <Code>shareToken</Code>
              </>,
              <>
                <Code>POST /api/configs/:id/clone</Code> — clone config into
                your account
              </>,
              <>
                <Code>GET /api/configs/:id/history</Code> — list version
                snapshots
              </>,
              <>
                <Code>GET /api/configs/:id/history/:snapshotId</Code> — get
                snapshot data
              </>,
            ]}
          />
          ...
          <UL
            items={[
              <>
                <Code>GET /api/collections</Code> — list your collections
              </>,
              <>
                <Code>POST /api/collections</Code> — create collection
              </>,
              <>
                <Code>PUT /api/collections/:id</Code> — update
              </>,
              <>
                <Code>DELETE /api/collections/:id</Code> — delete
              </>,
              <>
                <Code>GET /api/collections/:id</Code> — get collection with
                items
              </>,
              <>
                <Code>POST /api/collections/:id/items</Code> — add config
              </>,
              <>
          ...
          <UL
            items={[
              <>
                <Code>PUT /api/community/configs/:id/rate</Code> — upsert rating
                ({"{ value: 1–5 }"})
              </>,
              <>
                <Code>DELETE /api/community/configs/:id/rate</Code> — remove
                rating
              </>,
              <>
                <Code>POST /api/community/configs/:id/comments</Code> — post
                comment ({"{ body, parentId? }"})
              </>,
              <>
                <Code>
                  DELETE /api/community/configs/:id/comments/:commentId
                </Code>{" "}
                — delete own comment
              </>,
            ]}
          ...
          <UL
            items={[
              <>
                <Code>GET /api/notifications</Code> — list notifications
              </>,
              <>
  ...
  admin: [
    {
      ...
      content: (
        <>
          <P>
            The admin panel is accessible at <Code>/admin</Code>. It requires an{" "}
            <Code>X-Admin-Secret</Code> header or to be configured via the admin
            login modal on the page.
          </P>
          <H3>Features</H3>
          <UL
            items={[
              "Platform stats overview (total users, configs, views)",
              "Paginated user list with per-user config counts",
              "Paginated config list with filters (shared only, search)",
              "Force-unshare any published config",
              "Delete any user (cascades to their configs)",
              "Page-view breakdown by path for the past N days",
            ]}
          />
          <H3>Configuration</H3>
          <P>
            Set either <Code>ADMIN_SECRET</Code> (plaintext) or{" "}
            <Code>ADMIN_SECRET_HASH</Code> (bcrypt hash) in your server
            environment. Use <Code>npm run hash-secret</Code> to generate a hash
            from a plaintext secret.
          </P>
        </>
      ),
    },
  ...
  deployment: [
    {
      ...
      content: (
        <>
          ...
          <P>
            The production setup uses <Code>docker-compose.yml</Code>:
          </P>
          <UL
            items={[
              <>
                <Code>docker compose up -d</Code> — starts client (Nginx),
                server, and PostgreSQL containers
              </>,
              <>
                Set <Code>VITE_API_URL</Code> (client),{" "}
                <Code>DATABASE_URL</Code> (server), and{" "}
                <Code>ADMIN_SECRET_HASH</Code> (server) in <Code>.env</Code>{" "}
                files
              </>,
              <>
                The client container serves the built Vite bundle via Nginx with
                HTML5 routing support
              </>,
              <>
                The server container runs <Code>node dist/index.js</Code> after{" "}
          ...
          <P>
            <Code>docker compose -f docker-compose.dev.yml up</Code> mounts
            source code as volumes and runs both client (Vite HMR) and server (
            <Code>tsx watch</Code>) in watch mode.
          </P>
          <H3>Vercel</H3>
          <P>
            The client can be deployed to Vercel using <Code>vercel.json</Code>{" "}
            — all routes are rewritten to <Code>/index.html</Code> for
            client-side routing. Set <Code>VITE_API_URL</Code> as a Vercel
            environment variable pointing to your API server.
          </P>
          ...
          <UL
            items={[
              ...
              <>
                <Code>ADMIN_SECRET</Code> or <Code>ADMIN_SECRET_HASH</Code> —
                admin credentials (server)
              </>,
              <>
                <Code>PORT</Code> — server listen port (default 4000)
              </>,
              <>
                <Code>VITE_API_URL</Code> — API base URL (client, empty = same
                origin)
              </>,
            ]}
          />
