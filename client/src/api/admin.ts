...
/** Paginated list response from the admin reports endpoint. */
 ...
 * @param q          - Optional name search term.
 */
...
/**
 * Fetches a paginated list of all content moderation reports.
 *
 * @param secret - Admin secret.
 * @param page   - Page number (1-based).
...
export async function fetchAdminReports(
  ...
  const { data } = await apiClient.get<AdminReportList>(
...
export async function deleteAdminReport(
  ...
  await apiClient.delete(`/admin/reports/${id}`, adminHeaders(secret));
}
